# METI 네이티브앱 개발 에이전트 프롬프트 v2.3

> **최종 업데이트**: 2026-05-05  
> **이전 버전**: v2.2 (2026-05-02)  
> **변경 사항 요약**:  
> - 포인트 시스템 설계 확정 (개인/그룹 지갑, 1P=1원)  
> - 플랜 정책 확정 (free/pro/business + 구독 포인트 자동 지급)  
> - 인앱결제 정책 확정 (구독=인앱, 단건충전=웹 전용 — Netflix 방식)  
> - 웹 채널 구조 확정 (`/admin`, `/app/*`)  
> - 역할별 웹 분기 흐름 확정  
...
| 플랜 | 구독료 | 구독 지급 포인트 | 명함 한도 | 그룹 기능 | 그룹 관리자 기능 |
|------|-------|--------------|---------|---------|--------------|
| `free` | 0원 | 0 P | 3개 | 가입만 가능 | ❌ |
| `pro` | 미정 | 10,000 P/월 | 10개 | 생성·운영 가능 | ❌ |
| `business` | 미정 | 500,000 P/월 | 무제한 | 생성·운영 가능 | ✅ |

> - 구독료·포인트·한도는 슈퍼어드민에서 변경 가능 (`plan_configs`)
> - `business` 그룹 관리 기능: 멤버 통계, 행사 대량 초대, 레슨 출석 리포트 등
...
로그인 후 메인 화면
     ↓
GET /api/v1/groups (내가 admin인 그룹 확인)
     ↓
admin 그룹 존재 시 → 화면 상단에 [그룹 관리] 전환 버튼 표시
...
### 14-5. 명함 공개 설정 UI 가이드
- 기본 비공개 → 토글로 공개 전환
- 비공개: "나만 보기 · 공유 링크로만 접근"
- 공개: "전체 공개 · 검색/피드에 노출"
- 명함 목록에서 공개 상태 아이콘(🔒 / 🌐) 표시

### 14-6. 그룹 생성 신청 UI 가이드
...
| 0013 *(예정)* | `point_wallets`, `point_transactions`, `point_prices` 신규 + 기존 `plans`/`subscriptions`/`payments` 컬럼 확장 |
...
| Phase | 내용 | 서버 상태 |
|-------|------|---------|
| **현재** | 인증, 명함, 그룹, 행사, 채팅, 레슨 API | ✅ 완료 |
| **Phase 1** | 포인트 지갑/이력/차감 API | 설계 완료, 구현 예정 |
| **Phase 2** | 인앱결제 영수증 검증, 단건 충전 (웹) | PG사 선정 후 |
| **Phase 3** | 파트너 서비스 포인트 연동 | 추후 |

---

*METI NativeApp Agent Prompt v2.3 — 2026-05-05*
