...
> **작성일**: 2026-05-05  
> **이전 버전**: v1.1 (포인트 시스템 + DB 충돌 해결)  
> **변경 내용**:  
> - 웹 URL 구조 확정 (`/app/*`, `/admin` 분리)  
> - 다중 그룹 관리 UX 추가 (`/app/groups` 그룹 목록 페이지)  
> - 역할 분기 로직 구체화 (JWT → 화면 라우팅)  
> - 슈퍼어드민(`/admin`) vs 그룹관리자(`/app/group/:id/*`) 완전 분리  
> - 웹 SPA 파일 구조 확정 (`src/web/admin.ts`, `src/web/app.ts`)  
...
| `business` | 미정 | 500,000 P | 무제한 | 생성·운영 가능 | ✅ |

> - 구독료·지급 포인트·한도는 **슈퍼어드민 `plans` 테이블에서 변경 가능**
> - `business` 그룹 관리 기능 = 슈퍼어드민 기능과 완전히 별개

### 4-2. 구독 포인트 유효기간 정책 (권장, 추후 변경 가능)
...
| `plans` | — | **확장**: monthly_points, point_expire_days 컬럼 추가 |
| `subscriptions` | — | **확장**: pg_provider, pg_subscription_id, period 컬럼 추가 |
| `payments` | `payment_orders` | **확장**: order_no, wallet_id, points_to_add 등 추가 |
...
CREATE TABLE IF NOT EXISTS point_transactions (
  ...
  type            TEXT    NOT NULL,
  -- 충전: charge_subscription, charge_purchase, charge_admin, charge_transfer_in
  -- 차감: use_card_extra, use_event_create, use_nfc_card, use_partner, use_transfer_out
  -- 소멸: expire
...
#### `subscriptions` 테이블 확장 (ALTER TABLE)

```sql
ALTER TABLE subscriptions ADD COLUMN pg_provider           TEXT;
...
```
public/static/
├── admin.js               슈퍼어드민 SPA 로직 (기존 1,194줄)
...
### 8-3. 앱 라우터 분기 로직 (`public/static/app.js`)
...
function router() {
  ...
  const path = hash.slice(1);  // '#/dashboard' → '/dashboard'
  ...
  if (path.startsWith('/group/')) {
    ...
    renderGroupPage(id, sub || 'dashboard');
  } else {
    switch (path) {
...
### 10-3. 앱 내 포인트 충전 금지 사항 (Apple/Google 정책)
...
| 플랜 설정 | `/admin/plan-configs` | 월 구독료·포인트·한도 설정 |
...
| v1.2 | 2026-05-05 | 웹 URL 구조 확정 (`/app/*` SPA), 다중 그룹 관리 URL 추가 (`/app/groups`), 슈퍼어드민·그룹관리자 완전 분리, 웹 파일 구조 확정, 앱 라우터 분기 로직 문서화 |

---

*METI Product Spec v1.2 — 2026-05-05*
